...
from .seg_common import onehot_to_rgb
...
default_config = {
        "model_type": 'tiny_yolo3_mobilenetv3small_lite',
        "weights_path": os.path.join('models',  'multeye', 'test.h5'),
        "pruning_model": False,
        "anchors_path": os.path.join(*['models', 'multeye', 'configs', 'tiny_yolo3_anchors.txt']),
        "classes_path": os.path.join(*['models', 'multeye',  'configs', 'voc_classes.txt']),
        "score" : 0.1,
        "iou" : 0.4,
...
class YOLO_np(object):
    ...
    def detect_image(self, image):
        ...
        #start = time.time()
        out_boxes, out_classes, out_scores,seg_out = self.predict(image_data, image_shape)
        #print('Found {} boxes for {}'.format(len(out_boxes), 'img'))
        #end = time.time()
        #print("Inference time: {:.8f}s".format(end - start))
        seg_array=onehot_to_rgb(seg_out)
        ...
        return Image.fromarray(image_array),Image.fromarray(seg_array)
    ...
    def predict(self, image_data, image_shape):
        ...
        if num_anchors == 5:
            ...
            #print(len(yolo_out))
            yolo_box=[yolo_out[0],yolo_out[1],yolo_out[2]]
            seg_out=yolo_out[3]
            out_boxes, out_classes, out_scores= yolo3_postprocess_np(yolo_box, image_shape, self.anchors, len(self.class_names), self.model_image_size, max_boxes=100)
        return out_boxes, out_classes, out_scores,seg_out
...
class YOLO(object):
    ...
    def _generate_model(self):
        ...
        #plot_model(inference_model, to_file='model_plot.png', show_shapes=True, show_layer_names=True)
        return inference_model

    def predict(self, image_data, image_shape):
        out_seg,out_boxes, out_scores, out_classes= self.inference_model.predict([image_data, image_shape])
        #out_boxes, out_scores, out_classes,out_seg= self.inference_model.predict([image_data, image_shape])
        ...
        return out_boxes, out_classes, out_scores, out_seg
    ...
    def detect_image(self, image):
        ...
        start = datetime.now()
        out_boxes, out_classes, out_scores, out_seg = self.predict(image_data, image_shape)
        end = datetime.now()
        diff = end-start
        
        seg_array=onehot_to_rgb(out_seg)
        #draw result on input image
        image_array = np.array(image, dtype='uint8')
        image_array = draw_boxes(image_array, out_boxes, out_classes, out_scores, self.class_names, self.colors)
        classes = [self.class_names[i] for i in out_classes]
        return diff.total_seconds(),  out_boxes, classes,  Image.fromarray(image_array), self.class_names,  Image.fromarray(seg_array)
    
    def detect_image_vid(self, image):
        if self.model_image_size != (None, None):
            assert self.model_image_size[0]%32 == 0, 'Multiples of 32 required'
            assert self.model_image_size[1]%32 == 0, 'Multiples of 32 required'
        ...
        print('Found {} boxes for {}'.format(len(out_boxes), 'img'))
        print("Inference time: {:.8f}s".format(end - start))
        #seg_array=onehot_to_rgb(out_seg[0])
        #draw result on input image
        image_array = np.array(image, dtype='uint8')
        image_array = draw_boxes(image_array, out_boxes, out_classes, out_scores, self.class_names, self.colors)
...
def detect_video(yolo, video_path, output_path=""):
    ...
    while True:
        ...
        image,seg_image = yolo.detect_image(image)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--objdir', type=str, default=os.getcwd(),
        help='dir to store object detections in'
    )

    parser.add_argument(
        '--segdir', type=str, default=os.getcwd(),
        help='dir to store segmentations in'
    )

    parser.add_argument(
        '--imgdir', type=str,
        help='dir where images are stored'
    )
...
#        if "input" in args:
        ...
        detect_img(yolo, args.imgdir, args.segdir, args.objdir)
    elif "input" in args:
