...
def letterbox_resize(image,image_seg, target_size, return_padding_info=False):
    """
    ...
    #Same for Segmentation Viz
    image_seg = image_seg.resize(padding_size, Image.BICUBIC)
    new_image_seg = Image.new('RGB', target_size, (128,128,128))
    new_image_seg.paste(image_seg, offset)
    if return_padding_info:
        return new_image,new_image_seg, padding_size, offset
    else:
        return new_image,new_image_seg


def random_resize_crop_pad(image,image_seg, target_size, aspect_ratio_jitter=0.3, scale_jitter=0.5):
    """
    ...
    new_image_seg = Image.new('RGB', (target_w, target_h), (0,0,0))
    new_image_seg.paste(image_seg, padding_offset)

    return new_image,new_image_seg, padding_size, padding_offset
...
def random_horizontal_flip(image,image_seg, jitter=.5):
    """
    ...
    return image,image_seg, flip


def random_vertical_flip(image,image_seg, jitter=.2):
    """
...
def random_mosaic_augment(image_data,image_data_seg, boxes_data, jitter=.1):
    """
    ...
    if not do_augment:
        return image_data,image_data_seg, boxes_data
    else:
        ...
        def get_mosaic_samples():
            ...
            for idx in random_index:
                ...
                random_seg_images.append(image_data_seg[idx])
                random_bboxes.append(boxes_data[idx])
            return random_images,random_seg_images,np.array(random_bboxes)
        ...
        for i in range(batch_size):
            images,seg_images, bboxes = get_mosaic_samples()
            ...
            area_0 = seg_images[0][:crop_y, :crop_x, :]
            area_1 = seg_images[1][crop_y:, :crop_x, :]
            area_2 = seg_images[2][crop_y:, crop_x:, :]
            area_3 = seg_images[3][:crop_y, crop_x:, :]
            ...
            new_seg_images.append(merged_image)


        new_images = np.stack(new_images)
        new_seg_images=np.stack(new_seg_images)
        new_boxes = np.array(new_boxes)
        return new_images,new_seg_images, new_boxes
...
def letterbox_resize_r(image, target_size, return_padding_info=False):
    """
    Resize image with unchanged aspect ratio using padding
...
def preprocess_image(image, model_image_size):
    ...
    resized_image = letterbox_resize_r(image, tuple(reversed(model_image_size)))
