...
from .models.yolo3_darknet import yolo3_body, custom_tiny_yolo3_body, yolo3lite_body, tiny_yolo3lite_body, custom_yolo3_spp_body
from .models.yolo3_mobilenet import yolo3_mobilenet_body, tiny_yolo3_mobilenet_body, yolo3lite_mobilenet_body, yolo3lite_spp_mobilenet_body, tiny_yolo3lite_mobilenet_body
from .models.yolo3_mobilenetv2 import yolo3_mobilenetv2_body, tiny_yolo3_mobilenetv2_body, yolo3lite_mobilenetv2_body, yolo3lite_spp_mobilenetv2_body, tiny_yolo3lite_mobilenetv2_body
from .models.yolo3_shufflenetv2 import yolo3_shufflenetv2_body, tiny_yolo3_shufflenetv2_body, yolo3lite_shufflenetv2_body, yolo3lite_spp_shufflenetv2_body, tiny_yolo3lite_shufflenetv2_body
from .models.yolo3_vgg16 import yolo3_vgg16_body, tiny_yolo3_vgg16_body
from .models.yolo3_xception import yolo3_xception_body, yolo3lite_xception_body, tiny_yolo3_xception_body, tiny_yolo3lite_xception_body, yolo3_spp_xception_body
from .models.yolo3_nano import yolo3_nano_body
from .models.yolo3_efficientnet import yolo3_efficientnet_body, tiny_yolo3_efficientnet_body, yolo3lite_efficientnet_body, yolo3lite_spp_efficientnet_body, tiny_yolo3lite_efficientnet_body
from .models.yolo3_mobilenetv3_large import yolo3_mobilenetv3large_body, yolo3lite_mobilenetv3large_body, tiny_yolo3_mobilenetv3large_body, tiny_yolo3lite_mobilenetv3large_body
from .models.yolo3_mobilenetv3_small import yolo3_mobilenetv3small_body, yolo3lite_mobilenetv3small_body, tiny_yolo3_mobilenetv3small_body, tiny_yolo3lite_mobilenetv3small_body
...
from .postprocess import batched_yolo3_postprocess, batched_yolo3_prenms, Yolo3PostProcessLayer
...
yolo3_tiny_model_map = {
    ...
    #'tiny_yolo3_darknet': [custom_tiny_yolo3_body, 20, 'models/multeye/test.h5'],
    #Doesn't have pretrained weights, so no need to return backbone length
...
def get_yolo3_train_model(model_type, anchors, num_classes, weights_path=None, freeze_level=1, optimizer=Adam(lr=1e-3, decay=0), label_smoothing=0, model_pruning=False, pruning_end_step=10000):
    ...
    y_seg = Input(shape=(None, None, 13), name='Seg_in')
 ...
 #   print('model layer number:', len(model_body.layers))
  ...
  #      print('Load weights {}.'.format(weights_path))
   ...
   #     print('Freeze the first {} layers of total {} layers.'.format(num, len(model_body.layers)))
    ...
    enet = dbuild(model_body, nc=13)

    output2 = Activation('softmax', name='output2')(enet)

    cr = CustomRegularization()([y_seg,output2])
    out=Add()([cr,model_loss])

    model = Model(inputs=[model_body.input, *y_true,y_seg], outputs=[out])

    loss_dict = {'location_loss':location_loss, 'confidence_loss':confidence_loss, 'class_loss':class_loss, 'Seg_Loss':cr, 'Detection_Loss':model_loss}
    add_metrics(model, loss_dict)
    # losses={'yolo_loss': lambda y_true, y_pred: y_pred,
    #         'output2':'categorical_crossentropy'
    #         }
    # lossWeights={'yolo_loss':0.0,
    #             'output2':1.0
    #             }
    # metrics={ 'output2':['Precision','Recall',dice_coef]
    #         }
    # target1 = output2  # shapes of output1 your target has
    # target2 = model_loss # shapes of output2 your target has
...
def get_yolo3_inference_model(model_type, anchors, num_classes, weights_path=None, input_shape=None, confidence=0.2):
    ...
    # enet = dbuild(model_body.get_layer('activation_31').output, nc=13)
    # output2 = Activation('softmax', name='output2')(enet)
    
    # model = Model(inputs=[model_body.input,image_shape], outputs=[output2])
    ...
    # model = Model([model_body.input, image_shape], [boxes, scores, classes,output2])
    '''create the inference model, for YOLOv3'''
    K.clear_session() # get a new session
    num_anchors = len(anchors)
    #YOLOv3 model has 9 anchors and 3 feature layers but
    ...
    model_seg=Model([model_body.input],[output2,model_body.output])

    if weights_path:
        model_seg.load_weights(weights_path, by_name=False)#, skip_mismatch=True)
        print('Load weights {}.'.format(weights_path))
    ...
    model = Model([model_seg.input, image_shape], [output2,boxes, scores, classes])
...
class CustomRegularization(Layer):
    ...
    def call(self ,x ,mask=None):
        ...
        bce = K.categorical_crossentropy(ld, rd)
        loss2 = K.sum(bce)
