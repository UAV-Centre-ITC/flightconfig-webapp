...
def get_ground_truth_data(annotation_line,seg_path,input_shape, augment=True, max_boxes=100):
    '''random preprocessing for real-time data augmentation'''
    line = annotation_line.split()
    base=os.path.basename(line[0])
    base=os.path.splitext(base)[0]
    seg=os.path.join(seg_path,base+'.png')
    image = Image.open(line[0])
    image_seg=Image.open(seg)
    image_size = image.size
    model_input_size = tuple(reversed(input_shape))
    boxes = np.array([np.array(list(map(int,box.split(',')))) for box in line[1:]])

    if not augment:
        new_image,new_image_seg, padding_size, offset = letterbox_resize(image,image_seg, target_size=model_input_size, return_padding_info=True)
        image_data = np.array(new_image)
        image_seg_data=np.array(new_image_seg)
        ...
        return image_data,image_seg_data, box_data

    # random resize image and crop|padding to target size
    image,image_seg, padding_size, padding_offset = random_resize_crop_pad(image,image_seg,target_size=model_input_size)

    # random horizontal flip image
    image,image_seg,horizontal_flip = random_horizontal_flip(image,image_seg)
    ...
    image,image_seg, vertical_flip = random_vertical_flip(image,image_seg)
    ...
    image_seg_data=np.array(image_seg)
    image_data = normalize_image(image_data)
    box_data = np.zeros((max_boxes,5))
    if len(boxes)>0:
        box_data[:len(boxes)] = boxes
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    '''
    #assert (true_boxes[..., 4]==6).all(), 'class id must be 6 for cars'
